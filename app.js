...
(function () {
  ...
  const TOPICS = [
    ...
    'Burgere',
    'KFC',
    'Det Arbeiderpartiet gjør feil er...',
    'Wayne Rooney var egentlig god han',
    'Fergekaia i Molde bør flyttes til...',
    'Faen me sjukt med Terje Næss altså',
    'Remi Næss sine sykkelferdigheter',
    'Hans Christian Flor',
    'Torbjørn Flor',
    'Holmerns dame',
    'Nass/Na-gramatikk',
    'Antall barn i familien Kolberg',
    'Det Norge mangler ift Tyskland ølkulturmessig',
    'Her tar folk se en øl altså',
    'Vurdering av glassenes estetikk',
    'Looting',
  ...
  function renderTopicList() {
    ...
    if (!el) return;
    el.innerHTML = TOPICS.map((t, i) => '<div class="topic-item">' + (i + 1) + '. ' + t + '</div>').join('');
  }

  function showRandomTopic() {
    const el = document.getElementById('conversationTopic');
    if (!el) return;
    const topic = TOPICS[Math.floor(Math.random() * TOPICS.length)];
    el.innerHTML = '<div class="conv-topic">' + topic + '</div>';
  }

  (function initConversationStarter() {
    const btn = document.getElementById('newTopicBtn');
    if (btn) btn.addEventListener('click', showRandomTopic);
    renderTopicList();
  })();
